...
from .data import generate_authorization_info, generate_gateway_api, generate_integration, generate_response_parameter_mapping
from mapa.gateway.parameter_mapping.parameter_mapping_model import CreateParameterMapping
import pytest
...
async def test_paging(fixture):
    ...
    async with fixture.client() as client:
        ...
        paging_response = await client.get(f"/api/gateway/parameter_mapping/?{query_string}")
...
async def test_find_error(fixture):
    """Find metodunu test eder, olmayan bir parameter_mapping id olduğu için 404 dönmesi gerekiyor. """

    async with fixture.client() as client:
        parameter_mapping_id = str(uuid4())
        response = await client.get(f"/api/gateway/parameter_mapping/{parameter_mapping_id}")
...
async def test_find_create_success(fixture):
    ...
    async with fixture.client() as client:
        ...
        integration_id = create_integration_response.json()["items"][0]["id"]
        assert create_integration_response.status_code == 201

        create_parameter_mapping: CreateParameterMapping = generate_response_parameter_mapping(
            200, integration_id)

        create_parameter_mapping_response = await client.post(
            "/api/gateway/parameter_mapping/",
            content=json.dumps([obj.model_dump(exclude_none=True) for obj in [create_parameter_mapping]], cls=JsonEncoder))

    assert create_parameter_mapping_response.status_code == 201
