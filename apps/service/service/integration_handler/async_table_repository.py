...
class TableDesc:
    ...
    def _split_table_name(self, full_name: str) -> Dict[str, str]:
        ...
        return {"schema": "" if len(parts) == 1 else parts[0], "table": parts[-1]}


class AsyncTableRepository:
    ...
    def __init__(self, db: AsyncDatabase, table_desc: TableDesc) -> None:
        ...
        self.table: Table = None  # type: ignore

    async def load_instance(self):
        self.table = await self.__create_table_instance(self._table_desc)
    ...
    async def count_recursive(self, query_args: QueryArgs) -> int:
        ...
        async with self._db.session() as session:
            ...
            result_query = (
                session.query(recursive_query)
                .distinct()
                .order_by(nullsfirst(asc(recursive_query.c[parent_col_name])))
            )
    ...
    async def find_recursive(self, query_args: QueryArgs) -> List[Any]:
        # Kök tabloya ait etiket
        alias = self.__generate_alias(self.table)
            ...
            if query_args.order:
                ...
                for field, order in orders.items():
                    result_query = result_query.order_by(
                        asc(recursive_query.c[field])
                        if order == "asc"
                        else desc(recursive_query.c[field])
                    )
    ...
    async def find(self, query_args: QueryArgs) -> List[Any]:
        """Verilen sorgu parametrelerine göre modeli sorgular"""
        ...
        _, data = await self.__execute_statement(stmt, {})

        # Dönüş değeri json modele dönüştürülür
        return [self.__generate_nested_dict(row) for row in data]

    async def paging(self, query_args: QueryArgs) -> PagingResult[Any]:
        ...
        if self._table_desc.parent_col_name:
            ...
            if query_args.where is None or len(query_args.where) == 0:
                query_args.where = [
                    Filter(field=self._table_desc.parent_col_name, op=FilterOp.NULL)
                ]
            ret_val.total = await self.count_recursive(query_args)
            if ret_val.total > 0:
                ret_val.items = await self.find_recursive(query_args)
        else:
            ret_val.total = await self.count(query_args)
            if ret_val.total > 0:
                ret_val.items = await self.find(query_args)
        return ret_val
    ...
    async def create(
        ...
        if any(c.name == "created_at" for c in self.table.columns):
            values = list(map(lambda x: {**x, "created_at": datetime.now()}, values))
        ...
        if self._db.engine.dialect.use_insertmanyvalues == False:
            affected, items = await self.__execute_insert_single(self.table, values)
        else:
            affected, items = await self.__execute_statement(stmt, None)

        return ActionResult(success=True, affected=affected, items=items)
    ...
    async def update(
        ...
        if any(c.name == "updated_at" for c in self.table.columns):
            values = list(map(lambda x: {**x, "updated_at": datetime.now()}, values))
        ...
        affected, items = await self.__execute_statement(stmt, values)

        return ActionResult(success=True, affected=affected, items=items)

    async def update_with_query(
        self, query_args: QueryArgs, data: Dict[str, Any]
    ) -> ActionResult[Any]:
        """QueryArg verilen kaydı günceller"""
        ...
        affected, items = await self.__execute_statement(update_stmt, None)

        return ActionResult(success=True, affected=affected, items=items)

    async def delete(self, query_args: QueryArgs) -> ActionResult[Any]:
        """QueryArg verilen kaydı günceller"""
        if not query_args.where:
            raise ValueError("QueryArgs without where clause")
        ...
        delete_stmt = delete(self.table).where(
            self.table.primary_key.columns[0].in_(select_stmt.scalar_subquery())
        )

        affected, items = await self.__execute_statement(delete_stmt, None)
    ...
    def __create_field_list(self, query_args: QueryArgs) -> List[str]:
        ...
        def extract_fields(
            field_list: List[str], filter_list: List[Union[Filter, FilterGroup]]
        ):
            for filterx in filter_list:
                if filterx.type == "filter":
                    if filterx.field not in field_list:  # type: ignore
                        field_list.append(filterx.field)  # type: ignore
                else:
                    extract_fields(field_list, filterx.filters)  # type: ignore
        ...
        if query_args.model:
            model_fields = []
            self.__extract_fields_from_model(model_fields, query_args.model, None)
            query_args.select = model_fields

        # select parametreleri eklenir
        if query_args.select:
            return_val.extend([x for x in query_args.select if x not in return_val])
    ...
    async def __create_table_instance(self, table_desc: TableDesc) -> Table:
        ...
        async with self._db.engine.begin() as conn:
            ...
            table_info = await conn.run_sync(check_table)
    ...
    def __find_relations(
        self, start_alias: Any, expand: List[str]
    ) -> OrderedDict[str, Any]:
        ...
        for rel in expand:
            ...
            for field_name in field_names:
                fk_constraint = self.__find_fk_contraint(from_alias.element, field_name)
    ...
    def __apply_filter(
        ...
        def apply_filter(filter_list: List, filtery: Filter | FilterGroup, join_grps):
            if filtery.type == "filter":
                parts = filtery.field.split(".")  # type: ignore
                field_name = parts[-1]
                filter_attr = None
                if len(parts) == 1:
                ...
                if filter_attr is None:
                    raise ValueError(f"{filtery.field} field not found")  # type: ignore

                # Filtreler uygulanır
                cast_value = (
                    filtery.value  # type: ignore
                    if filtery.value is None  # type: ignore
                    else self.__cast_field_value(filter_attr, filtery.value)  # type: ignore
                )  # type: ignore
                match filtery.op:  # type: ignore
                    case FilterOp.EQUAL:
                    ...
                    case FilterOp.NOT_ILIKE:
                        filter_list.append(filter_attr.not_ilike(cast_value))
                    ...
                    case FilterOp.BETWEEN:
                        lower = cast_value[0]  # type: ignore
                        upper = cast_value[1]  # type: ignore
                        filter_list.append(between(filter_attr, lower, upper))
            else:
                sub_filter_list = []
                for filterz in filtery.filters:  # type: ignore
    ...
    def __cast_field_value(self, filter_attr: Any, value: Any) -> Any:
        def is_iso_datetime(s: str):
            try:
                ...
                if len(s) >= 8:
                    if (
                        re.match(Util.DDMMYYYHHmmssPattern, s)
                        or re.match(Util.DDMMYYYPattern, s)
                        or re.match(Util.DDMMYYYHHmmssSlashPattern, s)
                        or re.match(Util.DDMMYYYSlashPattern, s)
                    ):
        ...
        #         else create_value_object(value_type, value)
        #     )
    ...
    def __apply_sort(
        self, stmt, root_alias: Any, sort: Dict[str, OrderType], join_grps: Any
    ) -> Any:
    ...
    def __extract_fields_from_model(
        self, field_list: List[str], item: List[Any], parent_key: str | None
    ):
        ...
        for sub_item in item:
            if isinstance(sub_item, dict):
                ...
                field_list.append(
                    parent_key + "." + sub_item if parent_key else sub_item
                )

    def __extract_query_args(
        self, alias: Any, query_args: QueryArgs
    ) -> Tuple[Any, Any, Any]:
    ...
    def __create_fields(
        self, root_alias: Any, field_list: List[str], rels: OrderedDict[str, Any]
    ):
        fields = []
        for field in field_list:
            parts = field.split(".")
            if len(parts) > 1:
                path = ".".join(parts[:-1])
                join_grp = next(filter(lambda x: x[1]["path"] == path, rels.items()))
    ...
    async def __execute_statement(
        self, stmt: Any, params: Any, options: Any = None
    ) -> Tuple[int, Any]:
        if params:
    ...
    async def __execute_insert_single(
        self, table: Table, valuesList: List[Any], options: Any = None
    ) -> Tuple[int, Any]:
        stmt = insert(table)
        async with self._db.session() as session:
            row_count = 0
    ...
    def __get_parent_column(
        self, table: Table, parent_column: str
    ) -> Tuple[str, Column | None]:
        foreign_keys: Set[ForeignKeyConstraint] = table.foreign_key_constraints
        fk_list = list(filter(lambda x: x.referred_table == table, foreign_keys))
    ...
    def __find_fk_contraint(
        self, table: Table, col_name: str
    ...
    def __flatten_values(self, data: List[Any]) -> List[Any]:
        ...
        for row in data:
            ...
            for key, item in row.items():
                ...
                col = next(filter(lambda x: x.name == key, self.table.columns), None)
                if not col is None:
                    if isinstance(item, Dict):
                        fk_contraint = self.__find_fk_contraint(self.table, key)
                        if fk_contraint is not None:
                            fk_col_name = (
                                fk_contraint.referred_table.primary_key.columns[0].name
                            )
    ...
    def __set_locale(self, filter_attr: Any, locale_name: str) -> Any:
        if (
            isinstance(filter_attr.type, VARCHAR)
            or isinstance(filter_attr.type, NVARCHAR)
            or isinstance(filter_attr.type, TEXT)
            or isinstance(filter_attr.type, CHAR)
            or isinstance(filter_attr.type, NCHAR)
        ):
