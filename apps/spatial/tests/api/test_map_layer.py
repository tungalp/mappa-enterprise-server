...
from mapa.spatial.map_layer.map_layer_model import MapLayer, UpdateMapLayer
...
from .data import (generate_connection, generate_gateway_api, generate_layer,
                   generate_map, generate_map_layer, generate_route, tenant_id)
...
async def test_create_data_and_query_and_delete(fixture: SpatialFixture):
    ...
    empty_item = await map_layer_service.get(uuid4(), tenant_id=tenant_id)
    assert empty_item is None

    # Yeni 10 adet kayıt oluşturulur.
    # Yeni test kayıtları oluşturulur
    el_count = 1
    task_list = (map_layer_service.create(generate_map_layer(map_data.id, layer_definition_data[0].id), tenant_id=tenant_id)
    ...
    selected_items = await map_layer_service.find(QueryArgs(where=[
    ...
    test_value = "Test_XXX"
    first_item: MapLayer = items[0]

    updated_first_item = await map_layer_service.update(first_item.id, UpdateMapLayer(
    ...
    # ilk kayıt silinir
    first_row_deleted = await map_layer_service.delete(updated_first_item.id, tenant_id)
    ...
    deleted_row_count = await map_layer_service.delete_all(query_args_delete, tenant_id=tenant_id)
    assert deleted_row_count == len(items) - 1

    # Silinenler sorgulanır
    deleted_rows = await map_layer_service.find(query_args_delete, tenant_id=tenant_id)
