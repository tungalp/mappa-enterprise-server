...
from service.integration_handler.async_table_repository import (
    TableDesc,
    AsyncTableRepository as TableRepository,
)
...
def create_async_db_engine(db_url: str, is_async: bool) -> AsyncDatabase:
    """Verilen bağlantı parametrelerine göre veritabanı bağlantısı oluşturur."""
    return AsyncDatabase(db_url)


class AsyncAdHocQueryIntegrationHandler(IntegrationHandler):
    ...
    async def _execute_table(
        ...
        if not query is None:
            ...
            params["query"] = TypeAdapter(QueryArgs).validate_python(json.loads(query))
        ...
        match service_request.method:
            ...
            case MethodTypes.POST:
                ret_val = await self.__execute_table_post(
                    db_conn, table_desc, params, service_request.body
                )
            case MethodTypes.PUT:
                ret_val = await self.__execute_table_put(
                    db_conn, table_desc, params, service_request.body
                )
    ...
    async def _execute_sub_query(
        ...
        body = {"affected": row_count}
        match adhoc_conn.result_type:
            case SqlResultTypes.MULTI:
                body["data"] = [dict(item) for item in data][: adhoc_conn.max_count]
    ...
    def _create_db(self, params: Dict[str, Any]) -> AsyncDatabase:
        ...
        is_async = False
        if db_info.dialect.find("postgres") > -1:
            is_async = True

        return create_async_db_engine(db_url, is_async=is_async)

    async def __execute_table_get(
        self,
        db_conn: AsyncDatabase,
    ...
    async def __execute_table_post(
        ...
        result = await repo.create(values)
        return result

    async def __execute_table_put(
        self,
        db_conn: AsyncDatabase,
        table_desc: TableDesc,
        params: Dict[str, Any],
        ...
        if not query_args is None and isinstance(values, Dict):
            result = await repo.update_with_query(query_args, values)
        elif query_args is None:
            result = await repo.update(values)
        else:
            raise ValueError("Wrong update parameters", params, body)

        return result

    async def __execute_table_delete(
        self,
        db_conn: AsyncDatabase,
        table_desc: TableDesc,
        params: Dict[str, Any],
        ...
        return await repo.delete(query_args)
