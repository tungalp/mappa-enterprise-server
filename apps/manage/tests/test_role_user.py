...
from .conftest import generate_api,  generate_role, generate_role_user, generate_user
...
async def test_crud_success(fixture):
    ...
    async with fixture.client() as client:
        ...
        create_users_content = json.dumps([obj.model_dump(exclude_none=True)
                             for obj in create_users], cls=JsonEncoder)
        create_user_response = await client.post(f"/api/manage/user/", content=create_users_content)
        assert create_user_response.status_code == 201
        user_id = create_user_response.json()['items'][0]['id']

        create_role_user = generate_role_user(role_id, user_id)
        create_role_users = [create_role_user]
        create_role_users_content = json.dumps(
            [obj.model_dump(exclude_none=True) for obj in create_role_users], cls=JsonEncoder)
        create_role_user_response = await client.post(f"/api/manage/role_user/", content=create_role_users_content)

        create_role_user_id = create_role_user_response.json()['items'][0]['id']
        findresponse = await client.get(f"/api/manage/role_user/{create_role_user_id}")

        query_args: QueryArgs = QueryArgs(where=[
            Filter(field="id", op=FilterOp.EQUAL, value=create_role_user_id),
        ])

        query = query_args.model_dump_json()
        query_string = f"query={query}"
        pagingresponse = await client.get(f"/api/manage/role_user/?{query_string}")
        ...
        deleteresponse = await client.delete(f"/api/manage/role_user/?{delete_string}")

    assert create_role_user_response.status_code == 201
...
async def test_find_error(fixture):
    """Find metodunu test eder, olmayan bir role_user user id olduğu için 404 dönmesi gerekiyor. """

    async with fixture.client() as client:
        role_user_id = str(uuid4())
        response = await client.get(f"/api/manage/role_user/{role_user_id}")
