...
from .data import generate_namespace
...
async def test_paging(fixture):
    ...
    async with fixture.client() as client:
        ...
        paging_response = await client.get(f"/api/spatial/namespace/?{query_string}")
...
async def test_find_error(fixture):
    """Find metodunu test eder, olmayan bir namespace id olduğu için 404 dönmesi gerekiyor. """

    async with fixture.client() as client:
        namespace_id = str(uuid4())
        response = await client.get(f"/api/spatial/namespace/{namespace_id}")
...
async def test_find_create_success(fixture):
    """Create metodunu test eder, varolan bir namespace id olduğu için 200 dönmesi gerekiyor. """

    async with fixture.client() as client:
        create_namespace: CreateNamespace = generate_namespace()
        create_namespace_response = await client.post(
            "/api/spatial/namespace/",
            content=json.dumps([obj.model_dump(exclude_none=True) for obj in [create_namespace]], cls=JsonEncoder)
        )
        assert create_namespace_response.status_code == 201
        namespace_id = create_namespace_response.json()["items"][0]["id"]
        field_list = ["id", "name", "description", "title", "identifier"]
        response = await client.get(f'/api/spatial/namespace/{namespace_id}?field_list={json.dumps(field_list)}')
