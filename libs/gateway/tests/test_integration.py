...
from .conftest import GatewayFixture, generate_gateway_api, generate_authorization_info, generate_integration, tenant_id
from uuid import uuid4
from typing import Any, Dict
from unittest.mock import Mock
from .data import integration_select_list
...
async def test_create_data_and_query_and_delete(fixture: GatewayFixture):
    ...
    auth_info = await connection_info_service.create(CreateConnectionInfo(
        name="test_auth_info1",
        description="test_auth_info_desc1",
    ...
    el_count = 10
    task_list = (integration_service.create(generate_integration(gateway_api.id, auth_info.id), tenant_id=tenant_id)
    ...
    selected_items = await integration_service.find(QueryArgs(where=[
        Filter(field="id", op=FilterOp.IN, value=[
               str(item.id) for item in items])
    ], order={
        "name": "desc"
    },
        select=integration_select_list), tenant_id=tenant_id)
    assert len(selected_items) == el_count

    # ilk elemanın bir özniteliği değiştirilir
    test_value = "Test_XXX"
    first_item: Integration = items[0]

    updated_first_item = await integration_service.update(first_item.id, UpdateIntegration(
        name=test_value, description=test_value, connection=first_item.connection,context=first_item.context
    ), tenant_id=tenant_id)
    ...
    updated_el_count = await integration_service.update_all(query_args_update, UpdateAllIntegration(
        description=test_value_2,context=first_item.context
    ), tenant_id=tenant_id)
    assert updated_el_count == len(items)

    # ilk elemanı sorgulanır
    updated_first_item = await integration_service.get(first_item.id, tenant_id=tenant_id)
    assert updated_first_item.description == test_value_2

    # ilk kayıt silinir
    first_row_deleted = await integration_service.delete(updated_first_item.id, tenant_id)
    assert first_row_deleted is True

    # Son iki kayıt silinir
    last_item_ids = [cust.id for cust in items[-2:]]
    deleted_row_count = await integration_service.delete_by_ids(last_item_ids, tenant_id)
    ...
    deleted_row_count = await integration_service.delete_all(query_args_delete, tenant_id=tenant_id)
    assert deleted_row_count == len(items) - 3

    # Silinenler sorgulanır
    deleted_rows = await integration_service.find(query_args_delete, tenant_id=tenant_id)
